...
#define SHAPE_GIVE_PARAMETERS  start_x, start_y, line_width, color
	...
	protected: 
		...
		static int count;
		/*Каждая переменная член класса занимает память в объекте и хранит какую-то характеристику объекта.
		И эта характеристика принадлежит только конкретному объекту. Но иногда некоторые значения нужно сделать общими
		для всех объектов нашего класса, т.е. некоторые значения есть смысл вынести из объекта в класс
		чтобы эти значения хранились в одном месте в памяти и не занимали память в объектах.
		Такие переменные делают статическими "static". 
		Статическая переменная является общей для всех объектов определённого класса,
		хранится и занимает память в классе, а не в объектах.
		Статические переменные и константы можно проинециализировать только за пределами класса,
		за исключением статических констант типа int*/ 
	public:
	...
	public:
		...
		void draw()const override
		{
			...
			DeleteObject(hPen);
			//Функция DeleteObject удаляет логическое перо, кисть, шрифт, растровое изображение, 
			//область или палитру, освобождая все системные ресурсы,связанные с объектом.
			//После удаления объекта указанный дескриптор больше недействителен.
			DeleteObject(hBrush);
			ReleaseDC(hwnd, hdc);
			//Функция ReleaseDC освобождает контекст устройства (DC),
			//освобождая его для использования другими приложениями. 
			//Влияние функции ReleaseDC зависит от типа контроллера домена.
			//Он освобождает только общие контроллеры домена и контроллеры домена окон.
			//Он не влияет на класс или частные контроллеры домена.
